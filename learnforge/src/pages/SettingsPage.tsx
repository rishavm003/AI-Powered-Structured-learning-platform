...
export const SettingsPage: React.FC = () => {
  ...
  const { aiModel, setAiModel } = usePrefsStore();

  const models = [
    { id: 'nvidia_nim/moonshotai/kimi-k2-instruct', name: 'Kimi K2 Instruct (NVIDIA)' },
    { id: 'nvidia_nim/z-ai/glm4.7', name: 'GLM 4.7 (NVIDIA)' },
    { id: 'nvidia_nim/minimaxai/minimax-m2.5', name: 'MiniMax M2.5 (NVIDIA)' }
  ];
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-8">
      ...
      <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
        <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-200">AI Model Configuration</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Select the model powering your roadmap and quiz generation.</p>
          </div>
          <select
            value={aiModel}
            onChange={(e) => setAiModel(e.target.value)}
            className="px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
          >
            {models.map(m => (
